...
class Icd10Controller extends Controller
{
    ...
    public function lists(Request $request)
    {
        try {
            ...
            $data = Icd10::where('client_id', $this->clientId)
    ...
    {
        try {
            $data = Icd10::where('client_id', $this->clientId)->where('kode_icd', $id)->first();
    ...
    public function create(Request $request)
    {
        try {
            $data = new Icd10();
            $rec = Icd10::where('kode_icd', $request->kode_icd)->where('client_id', $this->clientId)->first();
            if($rec){
    ...
    public function update(Request $request)
    {
        try {
            ...
            $data = Icd10::where('kode_icd', $kodeIcd)->where('client_id', $this->clientId)->first();
            if (!$data) {
    ...
    {
        try {
            $data = Icd10::where('kode_icd', $id)->where('client_id', $this->clientId)->where('is_aktif', 1)->first();
            if (!$data) {
                return response()->json(['success' => false, 'message' => 'Data ICD tidak ditemukan.']);
            }

            $success = Icd10::where('kode_icd', $id)
    ...
    {
        try {
            Excel::Import(new ICD10Import,$request->file);
            return response()->json(['success' => true, 'message'  => 'Berhasil import Data ICD10']);
        }
        catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Gagal import data ICD10. Error Message: ' . $e->getMessage(), 'error' => $e->getMessage()]);
        }
    }
}
