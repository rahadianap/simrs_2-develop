...
class ReferensiController extends Controller
{
    ...
    {
        try {
            $data = Referensi::where('client_id', $this->clientId)->where('is_aktif', 1)->get();
            if(!$data) {
    ...
    public function listsRefPenunjang(Request $request)
    {
        try {
            $penunjangGroups = ['LAB','RAD','GIZI'];
            $data = Referensi::where('client_id', $this->clientId)
                ->whereIn('ref_group',$penunjangGroups)
                ->where('is_aktif', 1)
                ->get();
    ...
    public function newWorkspaceRef() {
        try {
