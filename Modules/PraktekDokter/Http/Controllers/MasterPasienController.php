        ...
        try {
            ...
            if($request->no_rm == '' || $request->no_rm == null) {                 
                ...
                if((int)$no_rm > 0) {
                    ...
                    return response()->json(['success' => false, 'message' => 'format no rekam medis hanya boleh berisi deretan angka tanpa huruf.']);
                }
            }

            //return response()->json(['success' => false, 'message' => $no_rm]);
            ...
            $data->pasien_id = $id;
            $data->no_rm = $no_rm;
            $data->is_pasien_luar = false;//$request->is_pasien_luar;
            $data->nama_pasien = $request->nama_pasien;
            $data->salut = '';//$request->salut;
            $data->nama_lengkap = strtoupper($request->nama_pasien);
