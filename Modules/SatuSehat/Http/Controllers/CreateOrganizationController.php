...
class CreateOrganizationController extends Controller
{
    ...
    public function lists(Request $request)
    {
        try{
            ...
            $data = Bridging::where('client_id',$this->clientId)->where('is_aktif',true)->where('resource', 'Organization')
    ...
    public function createOrganization(Request $request)
    {
        ...
        $body = [
            ...
            "partOf" => [
                ...
                "display" => $dataCentral->client_nama
            ]
        ];
        ...
        // check if data already bridged
        $exist = Bridging::where('client_id', $this->clientId)->where('local_resource_id', 'ILIKE', $request->unit_id)->orWhere('local_resource_name', 'ILIKE', $request->unit_nama)->where('is_aktif', 1)->first();
        if($exist){
            return response()->json(['success' => false,'message' => 'Data Unit Sudah Termapping. Silahkan Pilih Unit Lain.']);
        }
        ...
        $data->local_resource_id = $request->unit_id;
        $data->local_resource_name = $request->unit_nama;
        ...
        if(!$success) {
            ...
            return response()->json(['success' => false,'message' => 'ada kesalahan dalam mapping data unit.']);
        }
        DB::connection('dbclient')->commit();
        return response()->json(['success' => true,'message' => 'Unit berhasil dimapping.','data'=>$data]);
    }
