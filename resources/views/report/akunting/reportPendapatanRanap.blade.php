...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>LAPORAN PENDAPATAN RAWAT INAP</title>
        <!-- styling -->
    ...
    <body>
        ...
        <div class="uk-align-center" style="padding:0px !important;background-color:red;">
            <div style="font-size:14px;font-weight:bold;text-align:center;padding:0px !important;">LAPORAN PENDAPATAN RAWAT INAP</div>
        </div>
        ...
        <table class="uk-table uk-table-small standard-table" style="margin-top:-15px;margin:auto;width:100%;font-size:9px !important;">
            ...
            @foreach($penjaminData as $namaPenjamin => $transactions)
                @php
                    ...
                    $subtotalRanap = $transactions['RAWAT INAP'] ?? 0;
                    $total = $subtotalBHP + $subtotalLAB + $subtotalRadio + $subtotalRanap;
                @endphp
                <tr>
                    <td style="text-align:center;">{{ $rowNumber }}</td>
                    <td>{{ $namaPenjamin }}</td>
                    <td style="text-align:right !important;">{{ $subtotalRanap }}</td>
