<template>    
    <div class="uk-card">        
        ...
        <div class="uk-grid-small  uk-box-shadow-large" uk-grid style="margin:0;padding:0;">
            ...
            <div class="uk-width-expand" style="margin:0;padding:0;overflow:inherit;">
                <div style="margin:0;padding:0;">
                    <!-- default table. dengan isCheckboxAksi:true & isExpandable=true -->
                    <table v-if="isCheckboxAksi && isCheckboxColumns" ref="table-asset" class="uk-table hims-table uk-table-responsive1">
                        <slot name="tbl-header">
                            <thead>
                                <tr>
                                    <th :colspan="isExpandable ? 2 : 0" :width="isCheckboxAksi ? '10px' : '25px'">
                                ...
                                <tr>
                                    <th style="text-align: center; background-color: #fff;padding:1em;" :colspan="columns.length + 0" v-if="isLoading"> <!--fixing bug table ga bisa reload data otomatis kalo isExpandable=false  -->
                        ...
                        <slot name="tbl-body">
                            <tbody v-if="datas.data && datas.data.length > 0">
                                <!-- <base-row 
                                    :columns = "columns" 
                                    v-for="(dt, dt_index) in datas.data" 
                                    :isChecked="checkedStatus[dt_index]?.isChecked"
                                    @checked-changed="isChecked => rowCheckedChanged(dt_index, isChecked)" 
                                    ref="tb_rows" 
                                    :update-function="updateFunction" 
                                    :delete-function="deleteFunction" :rowData="dt" 
                                    :childNameIndex="childNameIndex" 
                                ...
                                <base-row 
                                    ...
                                    :childColumns="childColumns" 
                                    :isExpandable="config.isExpandable"
                                    :isCheckboxAksi="isCheckboxAksi"
                                    :isCheckboxColumns="isCheckboxColumns"
                                    :isAksiColumns="isAksiColumns"
                                    :isAksiDelete="isAksiDelete"
                                >
                                </base-row>
                            </tbody>
                            <tbody v-else>
                    ...
                    <!-- kondisi jika isCheckboxAksi=false & isExpandable=false-->
                    <table v-if="!isCheckboxAksi" ref="table-asset" class="uk-table hims-table uk-table-responsive1">
                        <slot name="tbl-header">
                            <thead>
                                <tr>
                                    <th :colspan="isCheckboxAksi && isExpandable ? 0 : 1" :width="isCheckboxAksi ? '50px' : '20px'"> </th>
                                    <th v-for="col in columns" v-bind:style="{ width: col.width, textAlign:col.textAlign,}">{{ col.title }}</th>
                                </tr>
                                <tr>
                                    <th style="text-align: center; background-color: #fff;padding:1em;" :colspan="columns.length + 0" v-if="isLoading">
                                        <p style="font-size:12px; font-weight: 500; font-style:italic;padding:0.5em; color:black;">
                                            <span uk-spinner></span>
                                            sedang mengambil data
                                        </p>
                    ...
                    <!-- kondisi jika isCheckboxColumns=false-->
                    <table v-if="!isCheckboxColumns" ref="table-asset" class="uk-table hims-table uk-table-responsive1">
                        <slot name="tbl-header">
                            <thead>
                                <tr>
                                    <th></th>
...
// import TableTicket from '@/Pages/AssetMaintenance/component/Ticket/table.vue';
// import TableSchedule from '@/Pages/AssetMaintenance/component/Schedule/table.vue';
...
export default {
    ...
    props : {
        ...
        */
        ...
        isCheckboxAksi:  { type:Boolean, require:true, default: true}, //menampilkan atau sembunyikan kolom checkbox dan kolom aksi
        isCheckboxColumns:  { type:Boolean, require:true, default: true}, //hanya menampilkan atau sembunyikan kolom checkbox saja
        isAksiDelete:  { type:Boolean, require:true, default: true}, // hanya menampilkan atau sembunyikan row Delete pada kolom Aksi

        // isAksi: { type:Boolean, require:false, default: false},
    },
    ...
    computed : {
        ...
        // reset checkbox pada title row, setelah submit filter is_aktif
    ...
    methods :{
        ...
        filterData(){
            ...
            // mapping checklist per row & menyesuaikan filterData is_aktif
            this.checkedStatus = this.data
                .filter(item => item.is_aktif)
                .map(item => ({
                index: item.dt_index,
                checked: this.checkedStatus.some(checked => checked.index === item.dt_index && checked.checked)
            }));
        ...
        // resetFilter() {
        // // Reset checkedStatus setelah submit filter
        //     this.checkedStatus = this.checkedStatus.filter(checked => this.data.some(item => item.dt_index === checked.index && c.index && item.is_aktif));
        // },
        ...
        retrieveData() {
            this.isLoading = true;
            this.initialCheckedStatus = this.checkedStatus.slice();
            this.CLR_ERRORS();  
