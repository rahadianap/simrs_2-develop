<template>
    <div :class="config.compClass" ref="containerListSelect" :style="config.compStyle" class="myListSelectContainer">
        ...
        <div class="uk-hidden"><input type="text" :name="searchName" v-model="value"></div>
...
export default {
    ...
    props : {
        ...
        fnSearch : { type : Function, required:true },
    ...
    methods : {
         ...
         * check if storeData is empty then component 
         * will retrieve data using actions that set in fnSearch.
         * to fill store data.
         */
        initialize(){
            if(this.storeData == null || this.storeData.length <= 0) {
                this.fnSearch().then((response) => {
        ...
        valueChange(newVal){
            /**
             * ada perubahan nilai komponen 
             * maka proses pencarian dimulai.
             * langkah :
             * 1. cari di storeData, jika tidak ada lanjut
             * 2. fetching data dengan keyword value
             */
        ...
        searchItemValue(){
            ...
            this.listindex = -1;
            if(this.items) {
                this.listindex = this.items.findIndex(x => x[this.valueField].toLowerCase() === this.value.toLowerCase());
                if(this.listindex >= 0) {
                    this.keyword = this.items[this.listindex][this.captionField];
                }
            }
        ...
        listData(init) { 
            this.listindex = -1;
            if(init) {
                if(this.value == null || this.value == '') { this.listindex = -1; this.keyword = ''; }
                else {
                    this.listindex = this.items.findIndex(x => x[this.valueField].toLowerCase().includes(this.value.toLowerCase()));
                    if(this.listindex >= 0){ this.keyword = this.items[this.listindex][this.captionField]; }
                }
            }
            else {
