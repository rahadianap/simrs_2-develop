            ...
            <base-table ref="tableInform"
                ...
                <template v-slot:tbl-body>
                    <tbody v-if="dataLists">
                        <row-map-inform v-for="dt in dataLists" 
                            :rowData="dt" 
                            :addCallback = newInformMapping                            
                            :removeCallback = removeInformMapping
...
import RowMapInform from '@/Pages/MasterData/InformedConsentUnitMapping/RowMapInform.vue';
import InformPicker from '@/Pages/MasterData/InformedConsentUnitMapping/InformPicker.vue';
...
export default {
    ...
    data() {
        return {
            ...
            pickerColumns : [
                { name : 'template_id', title : 'ID', colType : 'text', isCaption : false, },
                { name : 'template_nama', title : 'Nama', colType : 'text', isCaption : true, },
    ...
    methods : {
        ...
        ...mapActions('informedConsent',['createMapInformUnit','deleteMapInformUnit']),
        ...
        newInformMapping(data) {
            this.unitData = JSON.parse(JSON.stringify(data));
            this.$refs.informPicker.showModalPicker();
        },

        removeInformMapping(data) {
            if(data) {
                this.deleteMapInformUnit(data.unit_template_id).then((response) => {
                    alert (response.message);
                    if (response.success == true) {
                        this.$refs.tableInform.retrieveData();
                    }
                });
            }
        },

        pickerSelectedItem(data) {
            if(data) {
                let dt = JSON.parse(JSON.stringify(data));
                dt.unit_id = this.unitData.unit_id;
                this.createMapInformUnit(dt).then((response) => {
                    alert (response.message);
                    if (response.success == true) {
                        this.$refs.tableInform.retrieveData();
                    }
