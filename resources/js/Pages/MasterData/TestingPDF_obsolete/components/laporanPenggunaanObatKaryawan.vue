        ...
        <div class="uk-container paper-like" style="padding:0 1em 10em;" >
            <div style="position: sticky;top:10px;z-index:100;">
                <div class="uk-width-1-1 uk-text-right">
                    <input class="submit" type="submit" value="Print" style="margin-right:-120px" @click="printDiv('laporanPenggunaanObat')">
                </div>
            </div>
            <ul style="padding:0;margin:0;" id="laporanPenggunaanObat">
                    ...
                    <div style="padding:1em 0 1em 0;" class="uk-container">
                        <div style="padding:0; margin-top:1em; border-radius:10px;min-height:400px;" id="pdf-body-b">
                            ...
                            <div class="uk-align-center uk-margin-medium">
                                <div>
                                    <p class="uk-margin-small" style="font-size:16px;font-weight:bold;text-align:center;font-family: verdana-bold;letter-spacing:1.1px;text-decoration: underline">LAPORAN PENGGUNAAN OBAT</p>
                                    <p class="uk-margin-small" style="font-size:12px;font-weight:bold;text-align:center;">PENJAMIN : {{ this.penjamin }}</p>
                            ...
                            <table style="border:0.5px solid black;width:100%;font-size:11px;letter-spacing:0.5px;" class="uk-margin-small">
                                <thead>
                                    <tr>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Trans No.</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Nama</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Dokter</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Obat</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Satuan</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Qty</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-if="data_rekap" v-for="drekap in data_rekap">
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.trx_id }}</td>
                                        ...
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.item_nama }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:center;">{{ drekap.satuan }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.jumlah }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
export default {
    ...
    methods : {
        ...mapActions('pdf',['dataLapPenggunaanObatKaryawan']),
        ...
        callData(){
            this.dataLapPenggunaanObatKaryawan().then((response) => {
                if (response.success == true) {
                    this.data_rekap = response.data.data;
                    this.periode = response.data.periode;
                    this.penjamin = response.data.penjamin;
                }
                else { alert('data LAPORAN PENGGUNAAN OBAT KARYAWAN tidak ditemukan'); this.isLoading = false; }
            })
        }
