        ...
        <div class="uk-container paper-like" style="padding:0 1em 10em;" >
            <div style="position: sticky;top:10px;z-index:100;">
                <div class="uk-width-1-1 uk-text-right">
                    <input class="submit" type="submit" value="Print" style="margin-right:-120px" @click="printDiv('laporanRekapPasienRujukanLuar')">
                </div>
            </div>
            <ul style="padding:0;margin:0;" id="laporanRekapPasienRujukanLuar">
                    ...
                    <div style="padding:1em 0 1em 0;" class="uk-container">
                        <div style="padding:0; margin-top:1em; border-radius:10px;min-height:400px;" id="pdf-body-b">
                            ...
                            <div class="uk-align-center uk-margin-medium">
                                <div>
                                    <p class="uk-margin-small" style="font-size:16px;font-weight:bold;text-align:center;font-family: verdana-bold;letter-spacing:1.1px;text-decoration: underline">LAPORAN REKAP PASIEN RUJUKAN LUAR</p>
                                    <p class="uk-margin-small" style="font-size:12px;font-weight:bold;text-align:center;">bulan Juli 2023</p>
                                </div>
                            </div>
                            <table style="border:0.5px solid black;width:100%;font-size:11px;letter-spacing:0.5px;" class="uk-margin-small">
                                <thead>
                                    <tr>
                                        <th style="border-bottom:0.5px solid black;">No. REG</th>
                                        <th style="border-bottom:0.5px solid black;">No. RM</th>
                                        <th style="border-bottom:0.5px solid black;">Nama Pasien</th>
                                        <th style="border-bottom:0.5px solid black;">Unit Tujuan</th>
                                        <th style="border-bottom:0.5px solid black;">Unit Asal</th>
                                        <th style="border-bottom:0.5px solid black;">Dokter</th>
                                        <th style="border-bottom:0.5px solid black;">Dokter Pengirim</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-if="data_rekap" v-for="drekap in data_rekap">
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.no_reg }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.no_rm }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.nama_pasien }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.unit_nama }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.unit_asal }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.dokter_nama }}</td>
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.dokter_pengirim }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
export default {
    ...
    methods : {
        ...mapActions('pdf',['dataPasienAntiHIV','dataPasienRujukan']),
        ...
        callData(){
            this.dataPasienRujukan().then((response) => {
                if (response.success == true) {
                    this.data_rekap = response.data.data;
                    this.periode = response.data.periode;
                }
                else { alert('data rawat pasien rujukan luar tidak ditemukan'); this.isLoading = false; }
            })
        }
