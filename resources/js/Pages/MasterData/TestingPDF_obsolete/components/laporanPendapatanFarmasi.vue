<template>
    <div style="padding: 3em;">
        <div class="uk-container paper-like" style="padding:0 1em 10em;" >
            <div style="position: sticky;top:10px;z-index:100;">
                <div class="uk-width-1-1 uk-text-right">
                    <input class="submit" type="submit" value="Print" style="margin-right:-120px" @click="printDiv('laporanPendapatanFarmasi')">
                </div>
            </div>
            <ul style="padding:0;margin:0;" id="laporanPendapatanFarmasi">
                    ...
                    <div style="padding:1em 0 1em 0;" class="uk-container">
                        <div style="padding:0; margin-top:1em; border-radius:10px;min-height:400px;" id="pdf-body-b">
                            ...
                            <div class="uk-align-center uk-margin-medium">
                                <div>
                                    <p class="uk-margin-small" style="font-size:16px;font-weight:bold;text-align:center;font-family: verdana-bold;letter-spacing:1.1px;text-decoration: underline">LAPORAN PENDAPATAN FARMASI</p>
                                    <p class="uk-margin-small" style="font-size:12px;font-weight:bold;text-align:center;">{{ this.periode }}</p>
                                </div>
                            </div>
                            <table style="border:0.5px solid black;width:100%;font-size:11px;letter-spacing:0.5px;" class="uk-margin-small">
                                <thead>
                                    <tr>
                                        <th rowspan="2" style="border-bottom:0.5px solid black;text-align:center;">Tanggal</th>
                                        <th colspan="2" style="border-bottom:0.5px solid black;text-align:center;">IGD</th>
                                        <th colspan="2" style="border-bottom:0.5px solid black;text-align:center;">RWI</th>
                                        <th colspan="2" style="border-bottom:0.5px solid black;text-align:center;">RWJ</th>
                                        <th colspan="2" style="border-bottom:0.5px solid black;text-align:center;">Total</th>
                                    </tr>
                                    <tr>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Harga</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Diskon</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Harga</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Diskon</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Harga</th>
                                        <th style="border-bottom:0.5px solid black;text-align:center;">Diskon</th>
                                ...
                                <tbody>
                                    <tr v-if="data_rekap" v-for="drekap in data_rekap">
                                        <td style="border-bottom:0.5px solid black;">{{ drekap.tanggal }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.igd_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.igd_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.ri_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.ri_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.rj_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;">{{ drekap.rj_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ drekap.total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ drekap.total_diskon }}</td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">Grand Total</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.igd_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.igd_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.ri_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.ri_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.rj_total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.rj_diskon }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.total_harga }}</td>
                                        <td style="border-bottom:0.5px solid black;text-align:right;font-weight:bold;">{{ grand_total.total_diskon }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
export default {
    ...
    methods : {
        ...mapActions('pdf',['dataLapPendapatanFarmasi']),
        ...
        callData(){
            this.dataLapPendapatanFarmasi().then((response) => {
                if (response.success == true) {
                    this.data_rekap = response.data.data;
                    this.periode = response.data.periode;
                    this.grand_total = response.data.grand_total;
                }
                else { alert('data laporan pendapatan farmasi tidak ditemukan'); this.isLoading = false; }
            })
        }
