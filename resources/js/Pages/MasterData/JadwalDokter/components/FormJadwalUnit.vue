<template>
    <div id="formJadwalUnit" class="uk-modal" uk-modal>
            ...
            <div class="uk-container uk-container-medium">
                <form action="" @submit.prevent="submitJadwalDokter" class="uk-width-1-1 uk-grid-small" uk-grid style="padding:0;margin:0;">                    
                    <div class="uk-width-1-1@m" style="padding:0.5em 0.2em 0.5em 0.2em;margin:0;">
                        <h4 style="color:indigo;font-weight:500;">{{unit.unit_nama}}</h4>
                    </div>
                    <div class="uk-width-1-1@m" style="padding:0.2em;margin:0;">
                        <label class="uk-form-label" for="lokasi" style="font-size:12px;color:black;">Dokter*</label>
                        <div class="uk-form-controls">
                            <select class="uk-select uk-form-small" v-model="jadwal.dokter_unit_id" required @change="dokterSelected">
                                <option v-for="(dt,index) in unit.dokter" v-bind:value="dt.dokter_unit_id" v-bind:key="index">{{dt.dokter_nama}}</option>
                            </select>   
                        </div>
                    </div>
...
export default {
    ...
    methods : {
        ...
        dokterSelected(){
            let unit = this.unit.dokter.filter(item => { return item['dokter_unit_id'] === this.jadwal.dokter_unit_id }); 
            if(unit) { this.jadwal.dokter_id = unit[0].dokter_id; }
        },
        ...
        closeModal(){
            ...
            UIkit.modal('#formJadwalUnit').hide();
            return false;
        },
        ...
        newJadwalUnit(data){
            this.clearJadwalUnit(data);
            this.isUpdate = false;
            this.dataUnitPelayanan(data.unit_id).then((response)=>{
                if (response.success == true) {
                    this.fillDataUnit(response.data);
                    this.clearJadwalUnit();
                    this.isUpdate = false;
                    UIkit.modal('#formJadwalUnit').show();
                } else {
                    alert(response.message);
                }
            })
        ...
        clearJadwalUnit(){
            ...
            this.jadwal.unit_id = this.unit.unit_id;
