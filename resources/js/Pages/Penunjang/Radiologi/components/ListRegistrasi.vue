<template>
    <div>
        <header-page title="Radiologi" subTitle="daftar pasien radiologi"></header-page>
...
export default {
    ...
    methods : {
        ...mapActions('radiologi',['createRadiologi','deleteRadiologi','updateRadiologi','dataRadiologi','listsRegRad','dataRegRad']),
        ...
        deleteData(data) {
            this.CLR_ERRORS();
            if(confirm(`Proses ini akan menghapus data radiologi ini. Lanjutkan ?`)){
                this.deleteRadiologi(data.reg_id).then((response) => {
                    if (response.success == true) {
                        alert(response.message);
                        this.$refs.tableRadiologi.retrieveData();
                    }
