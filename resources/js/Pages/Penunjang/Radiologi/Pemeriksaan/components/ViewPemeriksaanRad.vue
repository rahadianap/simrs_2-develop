...
export default {
    ...
    methods : {
        ...
        ...mapActions('radiologi',['dataRadiologi','unConfirmRadiologi','confirmRadiologi']),
        ...
        konfirmasiReg(){
            ...
            if(confirm(`Proses ini akan mengubah status data registrasi ${this.registrasi.sub_trx_id} - ${this.registrasi.nama_pasien} dan memulai pengisian data. Lanjutkan ?`)){
                this.isLoading = true;
                this.confirmRadiologi(this.registrasi).then((response) => {
                    if (response.success == true) {
                        this.isLoading = false; this.isUpdate = false;
                        alert('data registrasi radiologi BERHASIL dikonfirmasi.');                        
                        this.clearRegistrasi();
                        this.$emit('confirmLabSuccess',response.data);
                        UIkit.modal('#modalViewRegistrasiRad').hide();
                    }
                    else { 
                        alert('data registrasi radiologi tidak berhasil dikonfirmasi.'); 
                        this.isLoading = false;
                    }
                })
            }
