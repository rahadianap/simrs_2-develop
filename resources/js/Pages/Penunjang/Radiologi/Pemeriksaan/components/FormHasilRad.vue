<template>
    <div class="" style="margin:0;padding:0;">        
                    ...
                    <div class="uk-container uk-container-large">                
                        ...
                        <div class="hims-form-container view-lab-container" style="padding:0 1.5em 2em 1.5em; margin:0;"> 
                            <div class="hims-form-subpage" style="padding:0.5em;">
                                ...
                                <p style="padding:0;margin:0;">{{ registrasi.no_rm }} - {{ registrasi.pasien_id }}</p>     
                            </div>
                            ...
                            <div class="uk-grid-small hims-form-subpage" uk-grid>                        
                                ...
                                <div class="uk-width-3-4@m uk-grid-small" uk-grid  style="padding:0.5em 0.5em 0.2em 0.5em;">
                                    ...
                                    <div class="uk-width-1-4@m">
                                        ...
                                        <div class="uk-form-controls">
                                            <input class="uk-input uk-form-small" v-model="registrasi.tgl_hasil" type="date" :min="minDate">
                                        </div>
                                    </div>
                                    <div class="uk-width-1-4@m">
                                    ...
                                    <div class="uk-width-1-4@m">
                                        ...
                                        <div class="uk-form-controls">
                                            <select v-model="registrasi.is_multi_hasil" class="uk-select uk-form-small" style="border:1px solid #cc0202;color:black;"> 
                                                <option :value=true >MULTI HASIL</option>
                                                <option :value=false >HASIL TUNGGAL</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="uk-width-1-4@m">
                            ...
                            <div v-if ="registrasi.is_multi_hasil == false" class="uk-grid-small hims-form-subpage" uk-grid>
                                <div class="uk-width-1-1@m uk-grid-small" uk-grid style="padding:0.5em 0.5em 0 0.5em;">
                                    <div class="uk-width-expand@m">
                                        <h5 style="color:indigo;padding:0;margin:0;">HASIL RADIOLOGI
                                            <span style="font-size:12px;font-style:italic;padding:0;margin:0 0 0 0.2em;">hasil pemeriksaan radiologi.</span>
                                        </h5>
                                    </div>
                                </div>
                                    ...
                                    <table class="uk-table uk-table-striped child-table tb-hasil-lab" style="padding:0;margin:0;">
                                        ...
                                        <tbody> 
                                            <tr v-for="res in registrasi.hasil">
                                                ...
                                                <td style="text-align:left;">
                                                    <p style="margin:0;padding:0;font-weight: 500;">{{res.expertise_nama}}</p>
                                                    <p style="margin:0;padding:0;font-size: 10px;">{{res.expertise_id}}</p>
                                                </td>
        ...
        <div>
            <modal-hasil-rad ref="modalHasilRad" 
                v-on:hasilRadUpdated = "ubahDataHasilRad">
            </modal-hasil-rad>
        </div>
    </div>
...
import ModalHasilRad from '@/Pages/Penunjang/Radiologi/Pemeriksaan/components/ModalHasilRad.vue';
...
export default {
    ...
    methods : {
        ...
        ...mapActions('radiologi',[
                'dataRadiologi', 'createRadiologi', 'updateRadiologi', 'deleteRadiologi',
                'confirmRadiologi', 'unConfirmRadiologi', 'dataHasilRadiologi', 'updateHasilRadiologi',
            ]),            
        ...
        modalClosed(){
            ...
            this.isUpdate = false;
            this.$emit('resultRadClosed',true);
        },
        ...
        viewData(refData) {
            if(refData) {
                ...
                this.isLoading = true;
                if(refData) {
                    this.dataHasilRadiologi(refData.trx_id).then((response) => {
                        if (response.success == true) {
                            this.fillRegistrasi(response.data);
                            this.isUpdate = true;
                            this.isLoading = false;
                        }
                        else { alert('data hasil radiologi tidak ditemukan'); this.isLoading = false; }
                    })
                }
            }
        },
        ...
        batalKonfirmasiReg(){
            ...
            if(confirm(`Proses ini akan Mengubah status data registrasi ${this.registrasi.trx_id} - ${this.registrasi.nama_pasien} kembali ke DAFTAR dan menghapus pencatatan jurnal. Lanjutkan ?`)){
                this.isLoading = true;
                this.unConfirmRadiologi(this.registrasi).then((response) => {
                    if (response.success == true) {
                        this.isLoading = false; this.isUpdate = false;
                        alert('konfirmasi registrasi radiologi BERHASIL dibatalkan.');
                        this.$emit('confirmRadCancel',response.data);
                    }
                    else { 
                        this.isLoading = false;
                        alert('konfirmasi registrasi radiologi tidak berhasil dibatalkan.'); 
                    }
                })
            }
        },
        ...
        submitHasilRad(){
            this.CLR_ERRORS();
            if(confirm(`Simpan hasil radiologi ${this.registrasi.trx_id} - ${this.registrasi.nama_pasien} ini ?`)){
                this.isLoading = true;
                this.updateHasilRadiologi(this.registrasi).then((response) => {
                    if (response.success == true) {
                        this.isLoading = false; 
                        this.isUpdate = true;
                        alert('hasil radiologi berhasil diubah.');                    
                    }
                    else { 
                        this.isLoading = false;
                        alert("hasil radiologi tidak berhasil disimpan."); 
                    }
                })
            }
        },
        ...
        updateData(data){
            if(data){
                this.$refs.modalHasilRad.showModal(data);
            }
        },

        ubahDataHasilRad(data) {            
        ...
        deleteData(data){
            this.CLR_ERRORS();
            if(confirm(`kosongkan hasil radiologi ${this.registrasi.trx_id} - ${this.registrasi.nama_pasien}. Lanjutkan ?`)){
                if(this.registrasi.is_multi_hasil) {
                    let item = this.registrasi.hasil.find(item => item.tindakan_id == data.tindakan_id );
                    if(item) {
