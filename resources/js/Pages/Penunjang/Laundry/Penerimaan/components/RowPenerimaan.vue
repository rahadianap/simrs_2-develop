    ...
    <tr v-if="rowData" style="border-bottom:1px solid #999;">
        ...
        <td style="width:180px;">
            <div class="uk-inline" style="margin:0;padding:0;">
                <a href="#" class="row-link-menu">{{rowData.linen_terima_id}}</a>
        ...
        <td style="width:120px;" class="">
            <p style="margin:0;padding:0;">{{rowData.tgl_terima}} {{rowData.jam_terima}}</p>
        </td>
        <td class="">
            <p class="uk-text-uppercase" style="margin:0;padding:0;">{{rowData.unit_pengirim}}</p>
        </td>
        <td class="uk-text-uppercase">
            <p style="margin:0;padding:0;">{{rowData.pengirim}}</p>
        </td>
        <td class="uk-text-uppercase">
            <p style="margin:0;padding:0;">{{rowData.catatan}}</p>
        </td>
        <td>
            <span v-if="rowData.is_infeksius" style="font-size:12px;" class="uk-label uk-label-danger">INFEKSIUS</span>
            <span v-else style="font-size:12px;" class="uk-label uk-label-success">NON INFEKSIUS</span>
        </td>
        ...
        <Transition>
            <td colspan="8" v-if="isExpanded" style="background-color:#f0f0f0;border-top:1px solid #efefef;">
                <div style="padding:0;margin:0;">
                    <h5 style="font-weight:500;padding:0;margin:0;">ITEM PENERIMAAN</h5>
                    <table class="uk-table uk-table-striped child-table" style="padding:0;margin:1em 0 0 0;">
                        <thead>
                            ...
                            <th class="child-table-header" style="width:100px; text-align:center;padding:1em;">Kondisi</th>                            
                        </thead>
                        <tbody>
                            <tr v-for="child in rowData.items">
                                <td style="width: 150px;">{{child.produk_id}}</td>
                                <td style="width: auto;">{{child.produk_nama}}</td>
                                <td style="width: 100px; text-align:center;">{{child.jml_terima}}</td>
                                <td style="width: 120px; text-align:center;">{{child.satuan}}</td>
                                <td style="width: 120px; text-align:center;">{{child.status}}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
