<template>
    <div class="" style="margin:0;padding:0;">        
        <div style="margin:0;padding:0;">
            <form action="" @submit.prevent="submisiRegPA" style="margin:0;padding:0;">
                        ...
                        <div class="uk-grid-small hims-form-subpage" uk-grid>                        
                            ...
                            <div class="uk-width-3-4@m uk-grid-small" uk-grid  style="padding:0 0.5em 0.2em 0.5em;">
                                <div class="uk-width-1-1@m">
                                    ...
                                    <div class="uk-form-controls">
                                        <input class="uk-input uk-form-small" v-model="registrasi.diag_klinis" type="text" style="color:black;" required>
                                    </div>
                                </div>
                                <div class="uk-width-1-4@m">
...
export default {
    ...
    methods : {
        ...
        ...mapActions('patologi',['createPatologi','deletePatologi','updatePatologi','updateHasilPatologi','dataPatologi','listsRegPa','dataRegPa']),
        ...
        penjaminSelected(data){
            if(data) {
                ...
                if(this.isUpdate) { this.submisiRegPA(); }
            }
        },
        ...
        submisiRegPA(){
            ...
            if(this.isUpdate) {
                this.isLoading = true;
                this.updatePatologi(this.registrasi).then((response) => {
                    if (response.success == true) {
                        this.isLoading = false;
                        this.isUpdate = true;
            ...
            if(confirm(`Proses ini akan menghapus data registrasi ${this.registrasi.trx_id} - ${this.registrasi.nama_pasien}. Lanjutkan ?`)){
                ...
                this.deletePatologi(this.registrasi.trx_id).then((response) => {
                    if (response.success == true) {
                        ...
                        alert('data registrasi patologi anatomi BERHASIL dihapus.');
                        this.modalClosed();
                    }
                    else { 
                        alert('data registrasi patologi anatomi tidak berhasil dihapus.'); 
                        this.isLoading = false;
                    }
                })
            }
        ...
        addPemeriksaan(dataVal){
            ...
            if(dataVal.length < 1) { return false; }
                ...
                addedTindakan.forEach(data => {
                    ...
                    let hargaRes = data.harga.find(item => item.buku_harga_id == this.registrasi.buku_harga_id);
            ...
            if(confirm(`Proses ini akan Mengubah status data registrasi ${this.registrasi.sub_trx_id} - ${this.registrasi.nama_pasien}. Lanjutkan ?`)){
                ...
                this.confirmRegistrasi(this.registrasi).then((response) => {
                    if (response.success == true) {
                        this.isLoading = false; 
                        this.isUpdate = false;
                        alert('data registrasi patoglogi anatomi BERHASIL dikonfirmasi.');
                        this.modalClosed();
                    }
                    else { 
                        alert('data registrasi patoglogi anatomi tidak berhasil dikonfirmasi.'); 
                        this.isLoading = false;
                    }
                })
            }
