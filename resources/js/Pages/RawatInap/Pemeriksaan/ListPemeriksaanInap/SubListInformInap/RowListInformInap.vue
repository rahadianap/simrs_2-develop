<template>
    <tr v-if="rowData" style="border-bottom:1px solid #ccc;"  v-bind:class="rowData.is_aktif != true ? 'inaktif':'' ">   
        ...
        <td style="padding:1em; font-weight: 500; font-size: 12px; color:black;">
            <p class="uk-text-uppercase" style="margin:0;padding:0;font-weight: 500;">{{rowData.asesmen_id}}</p>
        </td>
        <td style="padding:1em; font-weight: 500; font-size: 12px; color:black;">
            <p class="uk-text-uppercase" style="margin:0;padding:0;font-weight: 400;">{{rowData.tgl_assesmen}}</p>
        </td>
        <td style="padding:1em; font-size: 12px; text-align:center; color:black;">
            <p class="uk-text-uppercase" style="margin:0;padding:0;font-weight: 400;">{{rowData.template_nama}}</p>
        </td>
        <td style="padding:1em; font-weight: 500; font-size: 12px; color:black;">
        ...
        <td style="font-size: 12px; padding:1em; text-align:center; color:black;">
            ...
            <button @click.prevent="hapusAsesmen(rowData)">Hapus</button>
        </td>
    </tr>
    <!-- <tr>
        <Transition>
...
export default {
    ...
    methods : {          
        ...
        ...mapActions('medicalrecord',['deleteInformConsent']),
        ...
        hapusAsesmen(data) {
            this.CLR_ERRORS();
            if(confirm(`Proses ini akan menghapus data Asesmen. Lanjutkan ?`)){
                this.deleteInformConsent(data.asesmen_id).then((response) => {
