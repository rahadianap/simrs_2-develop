            ...
            <div style="margin:1em 0 0 0;padding:0;" class="uk-overflow-auto">                
                <div>
                    <table class="uk-table uk-table-striped hims-picker-table uk-table-responsive">
                        ...
                        <tbody>
                            <tr v-if="!isLoading" v-for="item in listItems">
...
export default {
    ...
    methods : {
        ...
        showModalPicker(tipe, keyPhrase, listSelection) {
            this.dataTipe = tipe;
            this.keyword = keyPhrase;
            this.isLoading = true;
            if(this.dataTipe == 'JKN') {
                this.listItems= listSelection;
                this.bufferItems= listSelection;
                this.isLoading = false;
            }
            else if(this.dataTipe == 'BPJS') {
                this.retrieveData();
            }
            else {
                this.isLoading = false;
        ...
        retrieveData() {
            this.CLR_ERRORS();
            this.listItems = null;
            if(this.dataTipe == 'BPJS') {
                if(this.keyword == null || this.keyword == '') { this.isLoading=false; return false; }
                else {
                    this.refBpjsDokter(this.keyword).then((response) => {
            ...
            else if(this.dataTipe == 'JKN') {
                let me = this;
                this.listItems = this.bufferItems.filter(function (el) { 
                    return el.namadokter.toLowerCase().includes(me.keyword.toLowerCase()) 
                    this.isLoading = false;
                }); 
            }
