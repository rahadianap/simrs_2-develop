<template>
        <template v-if="rowData.item_tipe == 'ITEM_RACIKAN' ">
            <tr style="border-bottom:1px solid #ccc;"  v-bind:class="rowData.is_aktif != true ? 'inaktif':'' ">
                ...
                <td style="width:40px;padding:0.4em; font-size: 12px; text-align:center; color:black;">{{ rowData.jumlah }}</td>
                <td style="width:40px;padding:0.4em; font-size: 12px; text-align:center; color:black;">
                    <input class="uk-input uk-form-small" type="number" v-model="rowData.jml_ambil" :max="rowData.jumlah" style="text-align: center;" @change="rowDataChange">
                </td>
        ...
        <template v-else-if="rowData.item_tipe == 'HEADER_RACIKAN' ">
            <tr style="border-bottom:1px solid #ccc;"  v-bind:class="rowData.is_aktif != true ? 'inaktif':'' ">
                ...
                <td style="width:40px;padding:0.4em; font-size: 12px; text-align:center; color:black;">{{ rowData.jumlah }}</td>
                <td style="width:40px;padding:0.4em; font-size: 12px; text-align:center; color:black;">
                    <input class="uk-input uk-form-small" type="number" v-model="rowData.jml_ambil" :max="rowData.jumlah" style="text-align: center;">
                </td>
                <td style="width:80px;padding:1em; font-size: 12px; text-align:right; color:black;">{{formatCurrency(rowData.subtotal)}}</td>
...
export default {
    ...
    methods : {
        ...
        rowDataChange(){
            if(this.rowData.jml_ambil > this.rowData.jumlah) {
                alert(`jumlah diambil tidak boleh melebihi jumlah resep, maksimum : ${this.rowData.jumlah}`);
                this.rowData.jml_ambil = this.rowData.jumlah;
            }
            this.rowData.diskon = 0;
            this.rowData.subtotal = this.rowData.jml_ambil * this.rowData.nilai * 1;
            this.dataChange();
        },
