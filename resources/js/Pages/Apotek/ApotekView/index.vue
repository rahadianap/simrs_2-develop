    ...
    <div class="uk-container uk-container-large" style="margin-top:1em; padding:1em 1em 2em 1em;">
        <div class="uk-grid uk-grid-small">
            ...
            <div class="uk-width-expand">
                <h4 style="font-weight:500;margin:0.2em 0 0 0;padding:0;" class="uk-text-uppercase">APOTEK OBAT</h4>
            </div>
        </div>
                ...
                <div style="padding:0;">
                    <table class="uk-table uk-table-striped1 child-table1" style="padding:0;margin:0;">
                        <thead>
                            ...
                            <th class="tb-header-reg" style="width:80px;text-align:center;color:white;background-color: #cc0202;">Jumlah</th>
...
import RowViewApotek from '@/Pages/Apotek/ApotekView/RowViewApotek.vue';
...
export default {
    ...
    methods : {
        ...
        ...mapActions('apotek',['dataApotek','realisasiApotek','cancelApotek']),

        getResepData(){
            this.dataApotek(this.trxId).then((response) => {
        ...
        cancelResep() {
            this.CLR_ERRORS();
            if(confirm(`Proses ini akan mengembalikan apotek ke DRAFT kembali. Data transaksi dan billing akan dihapus dan stock akan dikembalikan. Lanjutkan ?`)){
                this.cancelApotek(this.trxId).then((response) => {
                    if (response.success == true) {
                        alert('data transaksi apotek berhasil dikembalikan ke draft.');
                        this.isLoading = false;
                        this.$router.push({ name:'formApotek', params:{ trxId:this.trxId } });
                    }
                    else { 
                        this.isLoading = false;    
                        alert (response.message);
