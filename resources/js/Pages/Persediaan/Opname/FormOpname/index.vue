    ...
    <div class="uk-container">        
        <div class="uk-container" style="padding:0;">
            <div class="uk-grid-small hims-table-filter-container" uk-grid style="margin:0;padding:0.5em;"> 
                ...
                <!-- <div class="uk-width-1-1" style="margin:0;padding:1em 0 0 0.5em;">
                    <div class="uk-grid-small uk-box-shadow-large" uk-grid style="border-radius:5px;padding:0;">
                        ...
                        <button type="button" class="hims-table-btn uk-width-auto" @click.prevent="editDataOpname"><span uk-icon="search"></span> Refresh</button>
            ...
            <div style="margin:0.2em 0 0 0;padding:0;">
                ...
                <base-table ref="tableItemOpname"
                    :columns = "tbColumns" 
                    :config="configTable" 
                    :buttons = "buttons"
                    :urlSearch="urlSearch"
                    v-on:updateListDataTable="updateListItems">
                    <template v-slot:tbl-body>
                        <tbody v-if="soItems && soItems.length > 0">
                            <form-opname-row v-for="dt in soItems" :rowData="dt"></form-opname-row>
                        </tbody>
                    </template>
                </base-table>
            </div>
...
import FormOpnameRow from '@/Pages/Persediaan/Opname/FormOpname/FormOpnameRow.vue';
...
export default {
    ...
    data() {
        return {
            soItems : [],
            tbColumns : [
                { name : 'produk_id', title : 'ID',  colType : 'text'},
                { name : 'produk_nama', title : 'Depo', colType : 'text', },
                { name : 'jenis_produk', title : 'Catatan', colType : 'text', },
                { name : 'jml_total', title : 'Sistem', colType : 'text', },
                { name : 'total_so', title : 'Riil', colType : 'text', },
                { name : 'selisih_so', title : 'Selisih', colType : 'text', },
            ], 
            configTable : { isExpandable : false, filterType : 'SIMPLE', },
            buttons : [
                { title : 'Refresh', icon:'refresh', 'callback' : this.refreshData },
                { title : 'Simpan', icon:'file-text', 'callback' : this.saveData },
                { title : 'SO Selesai', icon:'lock', 'callback' : this.approveData },
            ],
            isLoading : false,
            isUpdate : false,
            opname : {
    ...
    methods :{
        ...
        initialize(){
            if(this.opnameId) { this.editDataOpname(this.opnameId); }
        },

        updateListItems(data) {
            //console.log(data);
            if(data) { this.soItems = data.data; }
        },
        ...
        refreshData(){
            this.$refs.tableItemOpname.retrieveData();
        },

        saveData() {
            this.opname.produk = JSON.parse(JSON.stringify(this.soItems));
            //JSON.parse(JSON.stringify(this.datas.data));
