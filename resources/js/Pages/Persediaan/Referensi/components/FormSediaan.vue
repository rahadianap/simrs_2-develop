    ...
    <div style="padding:0;margin:0;background-color: whitesmoke;">
        <div class="uk-grid uk-grid-small" style="padding:0 0 0.5em 0;margin:0;background-color: !!transparent;">
            <p class="uk-width-expand@m" style="padding:0 0.2em 0.2em 0.2em;margin:0;font-style: italic; font-size:14px;">
                referensi bentuk sediaan obat
            </p>
        </div>
        <div class="uk-card uk-card-default">
            <table class="uk-table uk-table-striped" style="padding:0;margin:0;">
                <thead>
                    <th style="padding:1em;margin:0;color:white;font-size:14px;font-weight:400;background-color:#cc0202;">SEDIAAN OBAT</th>
                    <th style="padding:1em;margin:0;color:white;font-size:14px;font-weight:400;background-color:#cc0202;">DESKRIPSI</th>
                    <th v-if="allowEdit" style="padding:1em;margin:0;color:white;font-size:14px;font-weight:400;text-align:center;background-color:#cc0202;">...</th>
                </thead>
                <tbody>
                    <tr v-if="allowEdit">
                        <td style="padding:0.5em;margin:0;color:black;font-size:12px;max-width:150px;">
                    ...
                    <tr v-for="dt in listData.json_data">
                        <td style="padding:1em;margin:0;color:black;font-size:12px;font-weight:500;width:150px;">{{dt.value}}</td>
                        <td style="padding:1em;margin:0;color:black;font-size:12px;font-weight:400;">{{dt.text}}</td>
...
export default {
    ...
    computed : {
        ...mapGetters(['errors']),
        ...mapGetters('refProduk',['jenisProdukRefs','isRefProdukExist','listRefProduk']),
    },
    methods : {
        ...mapActions('refProduk',['updateRefProduk']),
        ...
        addData(){
            if(this.refData.value == null || this.refData.value == '' || this.refData.text == ''|| this.refData.text == null){
                alert('Data tidak lengkap.');
                return false;
            }
            else {
                this.refData.value = this.refData.value.toUpperCase(); 
                this.refData.text = this.refData.text; 
